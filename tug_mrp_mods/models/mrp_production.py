...
class MrpProduction(models.Model):
    ...
    def action_update_from_bom(self):
        ...
        for production in self:
            ...
            # Only delete lines that are not done (just in case of partial weirdness, though usually all are open)
            moves_to_unlink = production.move_raw_ids.filtered(lambda m: m.state != 'done')
            moves_to_unlink.unlink()
            
            # 3. Re-generate moves from BOM
            if production.bom_id:
                # _get_moves_raw_values is the standard Odoo method to calculate components based on BOM and Qty
