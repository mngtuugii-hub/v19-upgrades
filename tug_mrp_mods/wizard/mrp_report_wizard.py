...
class MrpReportWizard(models.TransientModel):
    ...
    # We keep this field for logic, but we won't show it in the UI anymore
    action_type = fields.Selection([
        ('print', 'Print'),
        ('download', 'Download'),
        ('email', 'Email'),
        ('label', 'Print Labels (ZPL)') # Added option
    ], string='Action', default='print', required=True)

    # --- New Helper Methods for Single-Tap Buttons ---
    ...
    def action_confirm(self):
        ...
        # Both Print and Download trigger the standard report action
        if self.action_type in ['print', 'download']:
            ...
            # Using the corrected public method call
