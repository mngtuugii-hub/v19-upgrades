...
class SaleOrder(models.Model):
    ...
    def create_shopify_order_lines(self, lines, order_response, instance):
        ...
        for line in lines:
            ...
            # --- CHANGE START: Filter for unfulfilled items only ---
            
            # Get the quantity that is actually left to fulfill
            fulfillable_qty = float(line.get("fulfillable_quantity", 0.0))
            
            # If there is nothing left to ship for this line, skip it entirely
            if fulfillable_qty <= 0.0:
                continue
                
            # Use fulfillable_qty instead of the total line quantity
            quantity_to_import = fulfillable_qty
            # --- CHANGE END --
            # -
            # --- NEW CODE START ---
            # If the line has no SKU, do not create an order line for it.
            if not line.get("sku"):
                continue
            # --- NEW CODE END ---
            ...
            order_line = self.shopify_create_sale_order_line(line, product, quantity_to_import,
                                                             product.name, price,
                                                             order_response)
                ...
                for discount_allocation in line.get("discount_allocations"):
                    if instance.order_visible_currency:
                        discount_total_price = self.get_price_based_on_customer_visible_currency(
                            discount_allocation.get("amount_set"), order_response, 0.0)
    ...
    def check_mismatch_details(self, lines, instance, order_number, order_data_queue_line):
        ...
        for line in lines:
            # === ADD THIS CODE BLOCK ===
            # If the line has no SKU (like "Milk Option"), skip validation for it
            if not line.get('sku'):
                continue
            # ===========================
            ...
            if line.get('gift_card', False):
                # --- NEW CODE START ---
                # If the line has no SKU, skip the validation check entirely.
                if not line.get("sku"):
                    continue
                # --- NEW CODE END ---
    ...
    def prepare_tracking_numbers_and_lines_for_simple_tracking_order(self, moves, product_moves, picking):
        ...
        for kit_sale_line in kit_sale_lines:
            fulfillment_line_id = kit_sale_line.move_ids.filtered(lambda ml: ml.id in moves.ids)[
                0].shopify_fulfillment_line_id
            ...
            if not order:
                ...
                try:
                    ...
                    if self.env.ref('base.public_user').id == self.env.user.id:
                        ...
                        message = (f"The user has been changed from %s to %s for processing this order({order.name}) due to access right issue "
                                   f"during creation of the New Records such as product or product category.") % (self.env.ref('base.public_user').name, odoo_bot.name)
                        _logger.info(message)
    ...
    def create_shopify_order_return(self, refunds_data):
        ...
        for picking_id in orig_done_picking_ids:
            ...
            if self.shopify_instance_id.return_location_id:
                return_wiz.location_id = self.shopify_instance_id.return_location_id
