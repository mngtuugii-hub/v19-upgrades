...
class ShopifyPaymentReportEpt(models.Model):
    ...
    def create_bank_statement_lines_for_payout_report(self):
        ...
        for transaction in transaction_ids:
            if count >= 300:
                # Saving the records after 300 bank lines are generated.
                self.create_and_post_bank_statement_log_lones(log_lines)
                self._cr.commit()
                log_lines = []
                count = 0
            count += 1
    ...
    def create_and_post_bank_statement_log_lones(self,log_lines):
